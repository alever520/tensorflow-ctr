...
test_data_file_postfix=".shuffled.gz"
...
if [ "${is_python_auc}" == "Y" ];then
  if [ "${is_python_train_auc}" == "Y" ];then
    if [ -f ${train_data_01} ] && [ -f ${output_predict_of_train_data_path_01} ]; then
      auc="$(python  ${python_calauc} --project ${model_name}_train --time_tag $t_train_data --input_feature_data ${train_data_01} --input_score_data ${output_predict_of_train_data_path_01} ${python_auc_args} | awk '{print $0}')"
      echo -e "${auc}" >> ${file_auc}
    fi
  fi
  if [ "${is_python_test_auc}" == "Y" ];then
    if [ -f ${test_data_01} ] && [ -f ${output_predict_of_test_data_path_01} ]; then
      auc="$(python  ${python_calauc} --project ${model_name}_test --time_tag $t_test_data --input_feature_data ${test_data_01} --input_score_data ${output_predict_of_test_data_path_01} ${python_auc_args} | awk '{print $0}')"
      echo -e "${auc}" >> ${file_auc}
    fi
  fi
fi
