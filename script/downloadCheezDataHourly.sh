...
cheez_s3_data_base_path="s3://liveme.data.datastore/shortVideoBase/model"
...
do
  ...
  do
    hadoop fs -text ${cheez_s3_register_data_hourly_path}/part-* 2>/dev/null | python ${python_parser} | awk ' {print "reg\t"$0}'    > ${cheez_local_output_register_data_hourly_path}
    if [[ "$?" -ne 0 ]]; then
      echo "[E] download"
      ((try_num+=1))
...
#      cat ${cheez_local_output_register_data_hourly_path} | gzip > ${cheez_local_output_register_data_hourly_path}.gz
  ...
  while [[ try_num -lt ${try_max} ]]
  do
    hadoop fs -text ${cheez_s3_visitor_data_hourly_path}/part-* 2>/dev/null | python ${python_parser} | awk ' {print "vis\t"$0}'  > ${cheez_local_output_visitor_data_hourly_path}
    if [[ "$?" -ne 0 ]]; then
      echo "[E] download"
      ((try_num+=1))
      bash -x ${script_monitor} ${TELEPHONE} "[E] $PROJ_NAME $t_time failed ${try_num} times."
