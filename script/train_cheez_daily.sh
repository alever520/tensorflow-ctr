...
if [ "${is_python_auc}" == "Y" ];then
  if [ "${is_python_train_auc}" == "Y" ];then
    ...
    do
      ...
      if [ -f ${output_predict_of_train_data_path_01} ]; then
        auc="$(python  ${python_calauc} --project ${model_name}_train --time_tag $tmp_date_hour --input_feature_data ${train_data_01} --input_score_data ${output_predict_of_train_data_path_01} ${python_auc_args} | awk '{print $0}')"
        echo -e "${auc}" >> ${file_auc}
      fi
  ...
  if [ "${is_python_test_auc}" == "Y" ];then
    ...
    if [ -f ${output_predict_of_test_data_path_01} ]; then
      auc="$(python -u ${python_calauc} --project ${model_name}_test --time_tag $tmp_date_hour --input_feature_data ${test_data_01} --input_score_data ${output_predict_of_test_data_path_01} ${python_auc_args} | awk '{print $0}')"
      echo -e "${auc}" >> ${file_auc}
    fi
      tmp_date_hour=$(date +%Y%m%d%H -d "${tmp_date} ${tmp_hour}  +1 hour")
    done
