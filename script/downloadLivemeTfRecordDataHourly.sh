...
PROJ_NAME="download_liveme_tfrecord_data"

python_parser="${PYTHON_DIR}/yzy/data/parser/liveme.py"

liveme_s3_data_base_path="s3://liveme.data.datastore/shortVideoBase/livemetfrecordA_US"
liveme_s3_all_data_base_path="${liveme_s3_data_base_path}"
...
do
  ...
  liveme_s3_all_data_hourly_path="${liveme_s3_all_data_base_path}/${t_date}/${t_hour}"
  ...
  do
    hadoop fs -get ${liveme_s3_all_data_hourly_path}/part-r-00000  ${liveme_local_output_all_data_hourly_path}.shuffled
    if [[ "$?" -ne 0 ]]; then
      echo "[E] download"
      ((try_num+=1))
      bash -x ${script_monitor} ${TELEPHONE} "[E] $PROJ_NAME $t_time failed ${try_num} times."
    else
      #shuf ${liveme_local_output_visitor_data_hourly_path} > ${liveme_local_output_visitor_data_hourly_path}.shuffled
#      cat ${liveme_local_output_visitor_data_hourly_path} | gzip > ${liveme_local_output_visitor_data_hourly_path}.gz
      cat ${liveme_local_output_all_data_hourly_path}.shuffled | gzip > ${liveme_local_output_all_data_hourly_path}.shuffled.gz
      rm ${liveme_local_output_all_data_hourly_path}.shuffled
...
#  cat ${liveme_local_output_register_data_hourly_path} ${liveme_local_output_visitor_data_hourly_path} | shuf | gzip > ${liveme_local_output_all_data_hourly_path}.shuffled.gz
#  rm ${liveme_local_output_register_data_hourly_path}
#  rm ${liveme_local_output_visitor_data_hourly_path}
#  touch ${liveme_local_output_all_data_hourly_path}_SUCCESS
