"""Tensorflow Training code for CTR prediction"""
...
parser.add_argument(
    '--auc_multi_label_json', type=str, default='/tmp/census_data/feature.json',
...
def read_tfrecord_as_features(features_path, predicts_path):
    predicts = [[line.rstrip('\n') for line in open(predict_path)] for predict_path in predicts_path.split("#")]
    predicts_list = []
    for i in range(0, len(predicts)):
        predicts_list.append([tuple([float(l) for l in pred_str.split('\t')]) for pred_str in predicts[i]])

    predicts = [[sum(x) for x in zip(*tmp)] for tmp in zip(*predicts_list)]

    # predicts = [list(sum(p)) for multi_file_predicts in zip(*predicts) for p in zip([float(l) for l in single_file_predict.split('\t')] for single_file_predict in multi_file_predicts)]
    options = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.GZIP)
    example = tf.train.Example()
    res = []
    for (record, score) in zip(tf.python_io.tf_record_iterator(features_path, options=options), predicts):
        # score = [float(l) for l in predict.split('\t')]
...
if __name__ == '__main__':
    ...
    # label_info['SVDWELL'][1] = FLAGS.dwell_label_thresh
    # label_info['SVLIKE'][1] = FLAGS.like_label_thresh
    # label_info['SVSHARE'][1] = FLAGS.share_label_thresh
    # label_info['SVFOLLOW'][1] = FLAGS.follow_label_thresh
